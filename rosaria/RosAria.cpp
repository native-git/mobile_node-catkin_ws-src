...
class RosAriaNode
{
  ...
  protected:
    ...
    bool rotate_cb(roscpp_tutorials::TwoInts::Request& request, roscpp_tutorials::TwoInts::Response& response);
    bool move_cb(roscpp_tutorials::TwoInts::Request& request, roscpp_tutorials::TwoInts::Response& response);
  ...
  publish_aria_lasers(false)
{
  ...
  rotate_srv = n.advertiseService("rotate", &RosAriaNode::rotate_cb, this);
...
bool RosAriaNode::rotate_cb(roscpp_tutorials::TwoInts::Request& request, roscpp_tutorials::TwoInts::Response& response)
{
    //ROS_INFO("RosAria: Rotate request.");
    // rotate the requested number of degrees
    robot->lock();
    robot->comInt(13, request.a);
    sleep(1);
    robot->unlock();
  // todo could wait and see if motors do become disabled, and send a response with an error flag if not
    return true;
...
bool RosAriaNode::move_cb(roscpp_tutorials::TwoInts::Request& request, roscpp_tutorials::TwoInts::Response& response)
{
    //ROS_INFO("RosAria: Move request.");
    // move the desired number of millimeters
    robot->lock();
    robot->comInt(8, request.a);
    sleep(1);
    robot->unlock();
  // todo could wait and see if motors do become disabled, and send a response with an error flag if not
    return true;
