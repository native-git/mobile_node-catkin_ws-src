...
def launch_static((trans,rot)):
	command = "roslaunch grbils map_2_odom.launch "
	command += "x:=" +str(trans[0]) + " y:=" +str(trans[1]) + " qx:=0.0 qy:=0.0 qz:=" + str(rot[2]) + " qw:=" + str(rot[3]) +" >/dev/null &"
	print command
	os.system(command)
...
def refresh_odom_static():
	set_transform("map","pioneer",check_camera())
	set_transform("odom","base_link",lookup_odom())
	set_transform("pioneer","base_link",([0,0,0],[0,0,0,1]))
	kill_odom_transform()
	launch_static(get_transform("map","odom"))
...
with open('waypoints.csv','rb') as f:
	...
	for row in waypoints:
		...
		set_transform("map","target",(trans,rot))
		set_transform("odom","base_link",lookup_odom())
		set_transform("target","approach",([-0.5,0,0],[0,0,0,1]))
		set_transform("map","pioneer",check_camera())

		# Get position of target relative to camera (pioneer frame), and set it as odom_frame to target_relative
		set_transform("base_link","approach_relative",get_transform("pioneer","approach"))

		# Clear the costmaps
		clear_costmaps()

		# Approach the target from 0.5 m behind with the correct orientation
		...
		clear_costmaps()
		# Approach the target again
		move_base_client(get_transform("map","approach_relative"))

		# Now attempt to hit the waypoint
		# ------------------------------------------
		...
		while not at_target:
			...
			set_transform("odom","base_link",lookup_odom())
			set_transform("map","pioneer",check_camera())
			set_transform("map","base_link_initial",get_transform("map","base_link"))
			set_transform("base_link_initial","target_relative",get_transform("pioneer","target"))
			(t1,r1) = get_transform("base_link","target_relative")
			x = t1[0]
			y = t1[1]
