...
def check_camera():
	...
	while not rospy.is_shutdown():
		...
		if (any(abs(v)>=0.75 for v in v_vect) or abs(roll)>=10 or abs(pitch)>=10):
			c1 = True
		if (trans[0]<=-16 or trans[0]>=2 or trans[1]<=4 or trans[1]>=20 or abs(trans[2]-0.1)>=0.1):
			c2 = True
		if (c1 or c2):
			stable = False
		else:
...
def check_error():
	...
	while not rospy.is_shutdown():
		try:
			(trans,quat) = listener.lookupTransform('alfred','alfred/base_link',rospy.Time(0))
			trans = (trans[0],trans[1],0.0)
			euler = tf.transformations.euler_from_quaternion(quat)
			yaw = degrees(euler[2])
			if (any(abs(e)>=0.01 for e in trans) or abs(yaw)>=2) and stable and stationary:
				is_accurate = False
			else:
				is_accurate = True
		except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
...
def broadcast_static():
	...
	while not rospy.is_shutdown():
		...
		br.sendTransform(odom_trans,odom_rot,rospy.Time.now(),'alfred/odom','map')
		rate2.sleep()
