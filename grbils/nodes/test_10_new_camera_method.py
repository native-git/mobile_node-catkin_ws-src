...
rospy.init_node("test_10_new_camera_methodology")
...
def check_camera():
	...
	while not got_position:
		...
		if (abs(x-x_old)<= 0.00005) and (abs(y-y_old)<=0.00005) and (abs(yaw-yaw_old)<=0.00005):
...
with open('test_10_new_camera_methodology.csv', 'ab') as csvfile:
			...
			if abs(x) <= 0.002 and abs(y) <= 0.002 and theta <= 1:
				...
				while not facing_target:
					...
					if abs(theta) <= 1:
						facing_target = True
					if theta == theta_old:
						print "Last command too small, trying to overshoot..."
						theta = 3*theta
					if tries >= 10:
